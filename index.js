...
// Submit a review for a completed job (worker info derived server-side, not trusted from client)
...
// Check which jobIds already have reviews (for a batch of jobs)
...
app.post("/api/otp/send-customer", async (req, res) => {
  try {
    ...
    if (!job) {
      return res.status(404).json({ error: "No jobs found for this phone number." });
    }
...
// Verify OTP (shared by both worker and customer login)
